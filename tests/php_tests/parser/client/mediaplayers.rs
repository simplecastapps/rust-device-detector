...
fn basic(idx: usize, value: &mut Value) -> Result<()> {
    ...
    let test_version: &Value = &test_client["version"];
    let test_version: Option<String> = test_client["version"]
        .as_str()
        .map(|v| v.to_owned())
        .or_else(|| test_version.as_u64().map(|v| v.to_string()))
        .or_else(|| test_version.as_f64().map(|v| v.to_string()));
    let test_version = if test_version.as_deref() == Some("") {
        None
    } else {
        test_version
    };
    ...
    assert!(
        test_version.as_deref() == dd_version,
